...
const attributes = {
  'all': [
    ...
    'form', 'frameBorder', 'headers', 'height', 'hidden', 'high', 'href',
    'htmlFor', 'icon', 'id', 'integrity',
    'is', 'keyParams', 'keyType', 'label', 'lang',
    ...
    // Unknown
    'allowFullScreen', 'capture', 'cellPadding', 'cellSpacing',
    'inputMode', 'hrefLang',
  ],
  'area': ['alt'],
  'audio': ['autoPlay', 'loop'],
  'button': ['autoFocus'],
  'form': ['acceptCharset', 'action', 'autoComplete', 'formAction', 'formEncType', 'formMethod', 'formNoValidate', 'formTarget'],
  'img': ['alt'],
  ...
  getSuggestions(editor, token, options) {
    ...
    if (token.string.trim().length > 0 && token.type === 'attribute') {
      ...
      if (attributes[tagName]) {
        attributesSuggestions = attributesSuggestions.concat(attributes[tagName]);
      }
